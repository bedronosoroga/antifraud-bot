...
HISTORY_RISK_LABELS = {
    "none": "нет",
    "scarce": "нет (данных мало)",
    "unknown": "нет (не обнаружен в реестрах)",
    "elevated": "повышенный",
...
def format_history_entry(
    *,
    ...
    risk_label = HISTORY_RISK_LABELS.get(risk, risk)
    return (
        f"{emoji} <b>{code_display}</b> · {ts_str}\n"
        f"LIN: {lin} · EXP: {exp} · 🛡 риск: <b>{risk_label}</b>"
    )


def history_title(page: int) -> str:
    suffix = f" — стр. {page}" if page > 1 else ""
    return f"<b>История проверок{suffix}</b>"


def history_empty_text() -> str:
    return (
        "<b>История пуста.</b>\n"
        "Сделайте первую проверку — просто пришлите <b>код АТИ</b> (до 7 цифр)."
    )


def hero_banner() -> str:
    return "🛡 <b>Антифрод. Доверяй тем, кто проверен.</b>"


def start_onboarding_message() -> str:
    return (
        "🛡 <b>Привет! Я — Антифрод.</b>\n"
        "Помогаю проверять контрагентов по <b>коду АТИ</b> и понимать, насколько им можно доверять.\n\n"
        "🔗 <b>Отправь мне:</b>\n"
        "<b>код АТИ</b> (до 7 цифр), например <code>1234567</code>\n\n"
        "<b>Что вернётся:</b>\n"
        "• 📈 <b>Индекс перевозчика</b> и 📈 <b>индекс экспедитора</b> — сколько крупных и проверенных "
        "компаний уже работали с этим контрагентом.\n"
        "• ⚠️ <b>Факторы риска</b> — анализ профиля и активности компании на АТИ.\n"
        "• 🧾 <b>Краткий вывод</b> — понятная оценка и следующий шаг."
    )
...
def menu_help_text() -> str:
    return (
        "🔎 <b>Что я принимаю</b>\n"
        "• Код АТИ — до 7 цифр (пример: <code>1234567</code>).\n\n"
        "📊 <b>Что показываю в ответ</b>\n"
        "• 📈 Индекс перевозчика — ориентир по уникальным надёжным экспедиторам/грузовладельцам.\n"
        "• 📈 Индекс экспедитора — ориентир по уникальным надёжным перевозчикам.\n"
        "• ⚠️ Факторы риска — сигналы по активности и профилю на АТИ.\n"
        "• 🧾 Краткий вывод — сжатая интерпретация.\n\n"
        "🧠 <b>Как читать итог</b>\n"
        "• 🟢 Низкий риск — контрагент выглядит стабильно и прозрачно.\n"
        "• 🟡 Средний риск — стоит уточнить детали сделки.\n"
        "• 🔴 Высокий риск — осторожность: есть настораживающие сигналы.\n\n"
        "ℹ️ Каждый день в <b>00:00 по МСК</b> мы начисляем <b>+1 запрос</b>, если на этот момент у вас <b>0</b>. "
        "Это наш способ поддерживать базовую проверку безопасности."
    )
...
def request_prompt_text(balance: int) -> str:
    return (
        "🔎 <b>Сделать запрос</b>\n"
        "Пришлите <b>код АТИ</b> (до 7 цифр). Пример: <code>1234567</code>.\n"
...
def request_limit_text() -> str:
    return (
        ...
        "Чтобы продолжить, купите дополнительные запросы — доступ откроется мгновенно.\n"
        "Или подождите: завтра в <b>00:00 по МСК</b> добавится <b>1 бесплатный</b>."
    )


def free_requests_info() -> str:
    return (
        "🛡 <b>Как получать запросы бесплатно?</b>\n"
        "Помогайте друзьям и коллегам проверять контрагентов — за каждого приглашённого пользователя "
        "начисляется <b>+1 запрос</b>."
    )
...
def payment_packages_intro(balance: int) -> str:
    return (
        "💳 <b>Купить запросы</b>\n"
        "Выберите пакет — доступ откроется <b>мгновенно</b>.\n"
        f"<b>Доступно запросов сейчас:</b> {balance}"
    )


def payment_confirm_text(qty: int, price_rub: int) -> str:
    return f"Оплатить <b>{price_rub} ₽</b> за <b>{qty}</b> запросов?"


def payment_method_text() -> str:
    return (
        "💳 <b>Выберите способ оплаты</b>\n"
        "Доступ откроется <b>мгновенно</b> после успешной оплаты."
    )


def payment_pending_text() -> str:
    return "Перевёл(а) на платёжную страницу. Завершите оплату и вернитесь в бота."


def payment_success_text(granted: int, balance: int) -> str:
    return (
        f"Готово. Начислено <b>+{granted}</b>.\n"
        f"<b>Доступно запросов:</b> {balance}"
    )
...
def payment_error_text(kind: str) -> str:
    mapping = {
        "error": "Платёж не прошёл. Попробуйте ещё раз или обратитесь в поддержку.",
        "cancel": "Оплата отменена. Запросы не начислялись.",
        "timeout": "Похоже, оплата не была завершена. Если списания не было — попробуйте снова.",
        "duplicate": "Мы уже зафиксировали оплату по этому счёту. Проверьте баланс запросов.",
        "stars": "Недостаточно Telegram Stars для оплаты. Пополните баланс и попробуйте снова.",
    }
...
def profile_text(
    *,
    ...
    company_line = (
        f"<b>Мой код АТИ компании:</b> {company_ati}"
        if company_ati
        else "<b>Мой код АТИ компании:</b> не указан"
    )
    return (
        "👤 <b>Профиль</b>\n"
        f"<b>Ваш TG ID:</b> {tg_id}\n"
        f"<b>Дата регистрации:</b> {registered_at} ({since_phrase})\n"
        f"<b>Доступно запросов:</b> {balance}\n"
        "Если на момент <b>00:00 по МСК</b> у вас <b>0</b> запросов, мы автоматически начислим <b>+1</b>.\n"
        f"{company_line}"
    )


def profile_code_prompt(current: Optional[str]) -> str:
    current_line = f"Сейчас: {current}." if current else "Сейчас: не указан."
    return f"✏️ <b>Мой код АТИ компании</b>\n{current_line}\nОтправьте новый код (до 7 цифр)."


def profile_code_saved(ati: str) -> str:
    return f"Готово. Мой код АТИ: {ati}."
...
def err_need_digits_upto_7() -> str:
    ...
    return "Нужны 1–7 цифр. Пример: 1234567."
...
def too_many_requests() -> str:
    ...
    return "Слишком часто пытаетесь сделать запрос. Попробуйте позже."
...
def ref_program_text(
    *,
    ...
    next_tier_line = (
        f"1-я линия сейчас: <b>{percent}%</b>. До следующего уровня осталось <b>{next_tier_at}</b> "
        "приглашённых, которые оплатили."
        if next_tier_at is not None
        else "1-я линия сейчас: <b>{}%</b>. Вы на максимальном уровне.".format(percent)
    )
    return (
        "🤝 <b>Партнёрская программа</b>\n"
        "Помогайте друзьям и коллегам <b>проверять контрагентов</b> — и получайте вознаграждение.\n\n"
        "<b>Что вы получаете:</b>\n"
        "• <b>+1 запрос</b> за каждого приглашённого (начисляется сразу при старте по вашей ссылке).\n"
        "• <b>1-я линия</b> (процент с покупок):\n"
        "  1–4 — 15%, 5–14 — 25%, 15–49 — 35%, 50+ — 45%.\n"
        "• <b>2-я линия:</b> 5% с покупок рефералов ваших рефералов.\n\n"
        f"{next_tier_line}\n\n"
        f"<b>Реферальный баланс:</b> {fmt_rub_from_kop(balance_kop)}\n"
        f"<b>Всего заработано:</b> {fmt_rub_from_kop(total_earned_kop)}\n"
        f"1-я линия: {direct_total} (оплативших: {direct_paid})\n"
        f"2-я линия: {second_total} (оплативших: {second_paid})\n"
        f"Сегодня приглашено: {today_direct}\n\n"
...
def referrals_list_text(entries: list[dict]) -> str:
    if not entries:
        return "Пока нет рефералов. Отправьте ссылку друзьям и коллегам."
    lines = ["👥 <b>Мои рефералы</b>"]
    for item in entries:
        username = item.get("username")
        full_name = item.get("full_name") or "Без имени"
        paid = item.get("paid")
        status = "оплатил(а)" if paid else "не оплачивал(а)"
        if username:
...
def ref_create_tag_text() -> str:
    return (
        "✏️ <b>Создать свою ссылку</b>\n"
        "Укажите окончание ссылки (3–24 символа: латиница, цифры, подчёркивание).\n"
        "Примеры: <code>andrei</code>, <code>company</code>, <code>partner01</code>"
    )
...
def ref_withdraw_text(balance_kop: int) -> str:
    return (
        "💸 <b>Вывод средств</b>\n"
        f"Реферальный баланс: {fmt_rub_from_kop(balance_kop)}\n"
        "<b>Минимальная сумма вывода:</b> $10.\n"
        "<b>Комиссия платформы:</b> 8% (включает комиссию сети).\n\n"
        "Способы:\n"
        "• Telegram Stars\n"
        "• USDT (BEP20 / ERC20 / TON)\n\n"
        "Отправьте сумму и реквизиты (для USDT — сеть и адрес кошелька).\n"
...
def support_text() -> str:
    return "🆘 <b>Поддержка</b>\nОпишите, что случилось — мы поможем."
