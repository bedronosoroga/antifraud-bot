...
def b2b_ati_intro_text() -> str:
    return (
        ...
        "Мы можем показывать те же индексы и уровень риска <b>прямо в интерфейсе АТИ</b> — "
        "при поиске грузов, машин и в паспортах фирм.\n\n"
        "Это удобно, когда каждый день работаете в АТИ: сразу видно, "
        "с кем можно работать спокойнее, а по кому есть сигналы.\n\n"
        "Если по вашей рекомендации подключится компания (ваша или партнёров), "
        "вы получите отдельное вознаграждение за подключение.\n\n"
        "Нажмите <b>«Оставить заявку»</b> и в одном сообщении опишите, "
        "для какой компании нужен Антифрод в АТИ и как с вами связаться."
    )


def b2b_ati_lead_prompt_text() -> str:
    return (
        "✉️ <b>Заявка на Антифрод в АТИ</b>\n\n"
        "Напишите в одном сообщении:\n"
        "• компанию (название и код АТИ, если есть);\n"
        "• сколько человек работает в АТИ;\n"
        "• как с вами связаться (телефон, почта или @логин).\n\n"
        "Ответьте ниже одним сообщением."
    )


def b2b_ati_lead_ok_text() -> str:
    return (
        "✅ <b>Заявка отправлена</b>\n\n"
        "Мы получили ваше сообщение и свяжемся с вами, как только обработаем заявку.\n\n"
        "Если нужно что-то добавить — напишите это отдельным сообщением в чат с поддержкой @antifraud_support."
    )
...
def ref_program_text(
    *,
    ...
    next_tier_line = (
        f", до следующего уровня: ещё {next_tier_at} оплативших"
    ...
    return (
        ...
        "💼 <b>Антифрод в АТИ для компаний</b>\n"
        "Если по вашей рекомендации подключится компания, вы получите кешбек на реферальный баланс "
        "после первого оплаченного пакета подписки (1/3/6/12 месяцев).\n"
        "1 мес: 50% | 3 мес: 70% | 6 мес: 85% | 12 мес: 100%\n"
        "Кешбек за подключение начисляется один раз — после первого платежа компании.\n\n"
        "💰 <b>Статистика</b>\n"
        f"<b>Реферальный баланс:</b> {fmt_rub_from_kop(balance_kop)}\n"
        f"<b>Всего заработано:</b> {fmt_rub_from_kop(total_earned_kop)}\n\n"
        f"<b>1-я линия:</b> {direct_total} (оплативших: {direct_paid}, текущий процент: {percent}%{next_tier_line})\n"
...
__all__ = [
    ...
    "b2b_ati_intro_text", "b2b_ati_lead_prompt_text", "b2b_ati_lead_ok_text",
    # referral
