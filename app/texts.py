...
def format_history_entry(
    *,
    ...
    if risk == "critical":
        ...
        dot = "🟡"
        risk_text = "Повышенный"
    else:
        if lin >= 2 or exp >= 5:
            dot = "🟢"
            risk_text = "Не обнаружен"
        else:
            dot = "🟡"
    ...
    return (
        ...
        f"Перевозчик: {lin} · Экспедитор: {exp}\n"
        f"🛡 Риск: {risk_text}"
    )
...
def start_onboarding_message() -> str:
    return (
        ...
        "Помогу проверить компанию <b>по коду АТИ</b>, чтобы вы не рисковали сделкой.\n\n"
        "📎 <b>Что я проверяю</b>\n"
        "Код АТИ компании (до 7 цифр).\n"
        "Пример: <code>1234567</code>\n\n"
        "📊 <b>Что вы получите</b>\n"
        "<b>Индекс перевозчика</b> и <b>Индекс экспедитора</b> — это счётчики: они показывают, сколько надёжных крупных партнёров сотрудничали с компанией за год по нашим данным.\n"
        "<b>Факторы риска</b> — сигналы по активности и профилю на АТИ.\n"
        "<b>Краткий вывод</b> — простая оценка уровня доверия.\n\n"
        "🎁 На старте мы начислили вам <b>3 бесплатных запроса</b>. "
        "Отправьте код АТИ — я сформирую первый отчёт."
    )
...
def menu_help_text() -> str:
    return (
        ...
        "Пример: <code>1234567</code>\n\n"
        "📊 <b>Что вы получите</b>\n"
        "<b>Индекс перевозчика</b> — это число, показывающее, со сколькими надёжными заказчиками компания уже работала по нашим данным за год.\n"
        "<b>Индекс экспедитора</b> — это число, показывающее, со сколькими проверенными перевозчиками компания сотрудничала по нашим данным за год.\n"
        "<i>Например: перевозчик 8, экспедитор 5 — это значит, что у компании 13 подтверждённых связей с надёжными партнёрами.</i>\n"
        "<b>Факторы риска</b> — сигналы по активности и профилю на АТИ.\n"
        "<b>Краткий вывод</b> — простая оценка уровня доверия.\n\n"
        "📘 <b>Как читать результат риска</b>\n"
        "🟢 Низкий | 🟡 Средний | 🔴 Высокий\n\n"
        "ℹ️ Если у вас закончились запросы, в 00:00 по МСК мы начислим <b>1 бесплатный</b>, чтобы вы всегда могли сделать проверку."
    )


def report_actions_text() -> str:
    return "⚡️ <b>Быстрые действия</b>\nВыберите, что сделать дальше."
...
def payment_pending_text() -> str:
    return (
        "🧾 <b>Ссылка на оплату успешно сгенерирована</b>\n\n"
        "Нажмите <b>«Оплатить»</b> ниже и завершите оплату на странице YooKassa.\n\n"
        "После оплаты запросы начислятся автоматически в течение минуты."
    )


def payment_email_prompt_text() -> str:
    return (
        "📧 <b>Куда прислать чек?</b>\n\n"
        "Для оплаты картой нужен <b>e-mail</b>, на него отправим чек.\n"
        "Напишите почту в ответном сообщении.\n\n"
        "Спросим её только сейчас, при следующих оплатах больше не нужно будет вводить."
    )


def payment_email_saved_text(email: str) -> str:
    return (
        "📧 <b>Почту сохранили</b>\n"
        f"Чеки будем отправлять на: <code>{email}</code>.\n"
        "Если адрес нужно будет поменять — напишите в поддержку.\n\n"
        "Генерируем ссылку на оплату…"
    )


def payment_email_invalid_text() -> str:
    return (
        "Похоже, в адресе есть ошибка. Отправьте почту в формате <code>name@example.com</code> "
...
def profile_text(
    *,
    ...
    company_line = (
        f"<b>Ваш код АТИ:</b> {company_ati}"
        if company_ati
        else "<b>Ваш код АТИ:</b> не указан"
    )
    bonus_lines = []
    if not has_history:
        bonus_lines.append("🎁 При регистрации новым пользователям начисляем <b>3 бесплатных запроса</b>.")
    bonus_block = "\n".join(bonus_lines)
    return (
        (
            "👤 <b>Профиль</b>\n\n"
...
def b2b_ati_intro_text() -> str:
    return (
        "🏢 <b>Антифрод в АТИ для компаний</b>\n\n"
        "Подключаем индексы и риски <b>напрямую в АТИ</b> — "
        "в поиске грузов и машин, в паспортах контрагентов.\n\n"
        "Так ваша команда видит надёжность контрагентов "
        "там, где и работает каждый день — без перехода в бот.\n\n"
        "📞 <b>Как оставить заявку</b>\n"
        "Нажмите «Отправить номер» — можно отправить свой телефон или коллеги, с кем лучше обсудить подключение. Этого уже достаточно, чтобы мы с вами связались.\n"
        "Если хотите, потом сможете дописать пару деталей о компании в одном сообщении."
    )


def b2b_ati_contact_received_text() -> str:
    return (
        "✅ <b>Контакты получили</b>\n\n"
        "Этого уже достаточно, чтобы мы с вами связались "
        "и рассказали про подключение Антифрода прямо в АТИ.\n\n"
        "Если хотите, в <b>одном сообщении</b> дополнительно напишите:\n"
        "— код АТИ компании (если есть);\n"
        "— название компании и город;\n"
        "— сколько человек примерно работает в АТИ."
    )


def b2b_ati_details_saved_text() -> str:
    return (
        "✅ Спасибо! Информацию о компании получили.\n\n"
        "Мы свяжемся с вами, когда обработаем заявку."
    )


def method_page1_text() -> str:
    return (
        "ℹ️ <b>Методика расчёта индексов (1/3)</b>\n\n"
        "📊 <b>Что такое индекс</b>\n"
        "Простыми словами, <b>индекс</b> — это счётчик. Он показывает, сколько надёжных крупных партнёров уже работали с компанией по нашим данным.\n\n"
        "🚛 <b>Два вида индексов</b>\n"
        "• <b>Индекс перевозчика</b> — сколько надёжных заказчиков (экспедиторов и грузоотправителей) имели с компанией подтверждённые сделки за год.\n"
        "• <b>Индекс экспедитора</b> — сколько надёжных перевозчиков сотрудничали с этой компанией за год.\n\n"
        "На следующих шагах — откуда берутся данные и как читать эти цифры."
    )


def method_page2_text() -> str:
    return (
        "ℹ️ <b>Методика расчёта индексов (2/3)</b>\n\n"
        "📘 <b>Откуда берутся данные</b>\n"
        "Индексы рассчитываются на основе <b>открытых данных АТИ</b> и <b>закрытых источников</b>, которые предоставляют крупные компании и их службы безопасности.\n\n"
        "Мы учитываем только <b>крупных участников рынка</b> — ориентировочно <b>топ-100 экспедиторов и грузоотправителей</b> и <b>топ-500 перевозчиков России</b>. <b>Полный список и точная формула</b> не раскрываются, чтобы сохранить объективность анализа и избежать попыток манипуляции.\n\n"
    )


def method_page3_text() -> str:
    return (
        "ℹ️ <b>Методика расчёта индексов (3/3)</b>\n\n"
        "🧩 <b>Как пользоваться индексами</b>\n"
        "Индексы не делят компании на «хорошие» и «плохие» и не являются гарантией. "
        "Это ориентир, который помогает быстрее понять, есть ли у компании история работы с надёжными партнёрами.\n\n"
        "❕ По нашим наблюдениям, экспедиторы с <b>индексом экспедитора от 5 и выше</b> и перевозчики с <b>индексом перевозчика от 2 и выше</b> "
        "обычно уже имеют заметный опыт работы с надёжными контрагентами. "
        "Окончательное решение о сделке всегда остаётся за вами — индексы лишь дополнительная подсказка."
    )
...
def err_need_digits_upto_7() -> str:
    ...
    return (
        "⚠️ <b>Не удалось распознать код.</b>\n\n"
        "Код АТИ должен быть числом до 7 цифр.\n"
        "Отправьте корректный код — сразу проверим."
    )
...
def ref_program_text(
    *,
    ...
    next_tier_line = (
        f"1-я линия сейчас: <b>{percent}%</b>. До следующего уровня осталось <b>{next_tier_at}</b> приглашённых, которые оплатили."
        if next_tier_at is not None
        else f"1-я линия сейчас: <b>{percent}%</b>. Вы на максимальном уровне."
    )
    return (
        ...
        "<b>1-я линия</b> — процент с покупок приглашённых:\n"
        "1–4 оплативших: 15% | 5–14: 25% | 15–49: 35% | 50+: 45%\n"
        "<b>2-я линия</b> — 5% с покупок людей, которых пригласили ваши приглашённые.\n\n"
        f"{next_tier_line}\n\n"
        "💰 <b>Статистика</b>\n"
        f"<b>Реферальный баланс:</b> {fmt_rub_from_kop(balance_kop)}\n"
        f"<b>Всего заработано:</b> {fmt_rub_from_kop(total_earned_kop)}\n\n"
        f"<b>1-я линия:</b> {direct_total} (оплативших: {direct_paid})\n"
        f"<b>2-я линия:</b> {second_total} (оплативших: {second_paid})\n"
        f"<b>Сегодня приглашено:</b> {today_direct}\n\n"
        "🔗 <b>Ваша ссылка</b>\n"
        f"{link}\n\n"
...
def ati_checking_text(ati: str) -> str:
    return (
        f"🔎 <b>Проверяем код АТИ</b> <code>{ati}</code>…"
    )


def ati_invalid_code_text(ati: str) -> str:
    return (
        f"❌ Код АТИ <code>{ati}</code> не найден.\n\n"
        "Проверьте, нет ли ошибки в цифрах, и отправьте код ещё раз."
    )


def ati_no_data_or_error_text(ati: str) -> str:
    return (
        f"⚠️ Сейчас не получилось получить данные по коду АТИ <code>{ati}</code>.\n\n"
        "Запрос не списан — попробуйте ещё раз чуть позже или напишите в поддержку.\n\n"
        "Чаще всего так бывает, если код указан с ошибкой или по нему пока нет подтверждённых данных."
    )
...
def ref_withdraw_text(balance_kop: int) -> str:
    ...
    return (
        ...
        "Комиссия платформы: 8% (включает комиссию сети).\n\n"
        "Введите сумму, которую хотите вывести, <b>в рублях</b> (например: <code>1500</code>).\n"
        "Мы автоматически проверим минимальный порог по текущему курсу "
        "и покажем, сколько это примерно в USDT."
    )
...
def ref_withdraw_details_prompt(amount_rub: int, amount_usdt: float) -> str:
    return (
        "💸 <b>Заявка на вывод</b>\n\n"
        f"Сумма: {amount_rub} ₽ (≈ {amount_usdt:.2f} USDT по текущему курсу).\n\n"
        "Укажите реквизиты для выплаты:\n"
        "• Telegram Stars — @username.\n"
        "• USDT (BEP20 / ERC20 / TON) — сеть и адрес кошелька.\n\n"
...
def report_a(ati: str, lin: int, exp: int, tail: str) -> str:
    ...
    return (
        f"🟢 <b>Код АТИ {ati} успешно прошёл проверку.</b>\n\n"
        f"📈 <b>Индекс перевозчика</b>: {lin}\n"
        f"📈 <b>Индекс экспедитора</b>: {exp}\n\n"
        "🛡 <b>Чёрные списки</b>: риск не выявлен \n\n"
        f"{tail}"
    )
...
def report_b(ati: str, lin: int, exp: int) -> str:
    ...
    return (
        f"🟡 <b>Код АТИ {ati} обнаружен в нашем реестре проверок.</b>\n\n"
        f"📈 <b>Индекс перевозчика</b>: {lin}\n"
        f"📈 <b>Индекс экспедитора</b>: {exp}\n\n"
        "🛡 <b>Чёрные списки</b>: ⁉️ Повышенный риск\n\n"
...
def report_c(ati: str, lin: int, exp: int) -> str:
    ...
    return (
        f"🟡 <b>Код АТИ {ati} проверен.</b>\n\n"
        f"📈 <b>Индекс перевозчика</b>: {lin}\n"
        f"📈 <b>Индекс экспедитора</b>: {exp}\n\n"
        "🛡 <b>Чёрные списки</b>: не выявлен риск\n\n"
        "По нашим данным у компании есть подтверждённые связи, но их недостаточно для уверенной интерпретации."
    )
...
def report_d(ati: str, lin: int, exp: int) -> str:
    ...
    return (
        f"🔴 <b>Код АТИ {ati} не прошёл проверку.</b>\n\n"
        f"📈 <b>Индекс перевозчика</b>: {lin}\n"
        f"📈 <b>Индекс экспедитора</b>: {exp}\n\n"
        "🛡 <b>Чёрные списки</b>: ⛔️ Критический риск \n\n"
        "По данным нашей аналитики, есть риски. Оценка субъективна и может быть изменена при появлении благоприятных признаков."
    )
...
def report_e(ati: str) -> str:
    ...
    return (
        f"🟡 <b>Код АТИ {ati} найден, но подтверждённых связей в наших реестрах нет.</b>\n\n"
        "📈 <b>Индекс перевозчика</b>: 0\n"
        "📈 <b>Индекс экспедитора</b>: 0\n\n"
        "🛡 <b>Чёрные списки</b>: не выявлен риск\n\n"
        "По нашим данным у компании пока нет подтверждённых связей с крупными надёжными контрагентами."
    )
...
__all__ = [
    ...
    "profile_text", "profile_code_prompt", "profile_code_saved",
    "b2b_ati_intro_text", "b2b_ati_contact_received_text", "b2b_ati_details_saved_text",
    # referral
