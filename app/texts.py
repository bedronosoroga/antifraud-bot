...
def payment_email_prompt_text() -> str:
    return (
        "📧 <b>Один раз спросим почту для чека</b>\n\n"
        "По закону после оплаты должен идти чек.\n"
        "Сейчас — <b>первая оплата</b>, поэтому попросим ваш <b>e-mail</b>.\n"
        "Дальше будем отправлять чеки на этот адрес, при необходимости его можно поменять через поддержку."
    )
