...
def start_onboarding_message() -> str:
    return (
        ...
        "📎 <b>Что я проверяю</b>\n"
        "Код АТИ компании (до 7 цифр). Пример: <code>1234567</code>\n\n"
        "📊 <b>Что вы получите</b>\n"
        "<b>Индекс перевозчика</b> — показывает, со сколькими надёжными заказчиками компания работала за год.\n"
        "<b>Индекс экспедитора</b> — показывает, со сколькими проверенными перевозчиками компания сотрудничала за год.\n"
        "<b>Факторы риска</b> — сигналы по активности и профилю на АТИ.\n"
        "<b>Краткий вывод</b> — итоговая оценка уровня доверия.\n\n"
        "🎁 На старте мы начислили вам <b>3 бесплатных запроса</b> — просто отправьте первый код АТИ и получите отчёт."
    )
...
def menu_help_text() -> str:
    return (
        ...
        "Это наш способ поддерживать доверие и безопасность на рынке перевозок."
    )
...
def method_info_text() -> str:
    return (
        "ℹ️ <b>Методика расчёта индексов</b>\n\n"
        "<b>Индексы рассчитываются</b> на основе <b>открытых данных АТИ</b> и <b>закрытых источников</b>, "
        "которые предоставляют крупные компании и их службы безопасности. Мы объединяем эту информацию, чтобы "
        "оценить реальные связи между перевозчиками и грузоотправителями на рынке.\n\n"
        "🔹 <b>Индекс перевозчика</b> показывает, со сколькими надёжными заказчиками (экспедиторами и грузовладельцами) компания имела подтверждённые сделки за год.\n"
        "🔹 <b>Индекс экспедитора</b> показывает, со сколькими проверенными перевозчиками компания сотрудничала за год.\n\n"
        "В расчёт входят данные по <b>крупнейшим участникам рынка</b> — ориентировочно <b>топ-100 экспедиторов и грузовладельцев</b> "
        "и <b>топ-500 перевозчиков России</b>. <b>Подробная методика и состав выборки</b> не раскрываются, чтобы сохранить "
        "объективность анализа и исключить влияние извне."
    )
