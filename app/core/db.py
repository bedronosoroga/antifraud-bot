...
async def list_paid_subs_expiring_between(ts_from: datetime, ts_to: datetime) -> list[dict[str, Any]]:
    start = _ensure_datetime_utc(ts_from)
    end = _ensure_datetime_utc(ts_to)
    stmt = (
        select(
            subs.c.uid,
            subs.c.plan.label("plan"),
            subs.c.expires_at,
            subs.c.started_at,
            users.c.username,
            users.c.first_name,
        ...
        .join(users, users.c.id == subs.c.uid)
        .where(subs.c.plan != "none")
...
async def list_paid_subs_expiring_on(ts_day_start: datetime, ts_day_end: datetime) -> list[dict[str, Any]]:
    return await list_paid_subs_expiring_between(ts_day_start, ts_day_end)


async def list_trials_expiring_between(ts_from: datetime, ts_to: datetime) -> list[dict[str, Any]]:
    start = _ensure_datetime_utc(ts_from)
    end = _ensure_datetime_utc(ts_to)
    stmt = (
        select(
            free_grants.c.uid,
            free_grants.c.granted_at,
            free_grants.c.expires_at,
            free_grants.c.total,
            free_grants.c.used,
            users.c.username,
            users.c.first_name,
...
async def list_trials_expiring_within(ts_from: datetime, ts_to: datetime) -> list[dict[str, Any]]:
    return await list_trials_expiring_between(ts_from, ts_to)


def _day_bounds(day: datetime) -> tuple[datetime, datetime]:
    day_utc = _ensure_datetime_utc(day)
    start = day_utc.replace(hour=0, minute=0, second=0, microsecond=0)
    end = start + timedelta(days=1)
    return start, end


async def notif_was_sent(uid: int, kind: str, day: datetime) -> bool:
    day_start, day_end = _day_bounds(day)
    stmt = (
        select(func.count())
        .select_from(user_notifications)
        .where(user_notifications.c.uid == uid)
        .where(user_notifications.c.kind == kind)
        .where(user_notifications.c.sent_at >= day_start)
        .where(user_notifications.c.sent_at < day_end)
    )
    async with Session() as session:
        result = await session.execute(stmt)
